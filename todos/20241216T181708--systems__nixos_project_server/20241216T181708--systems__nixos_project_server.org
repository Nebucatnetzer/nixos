...
*** NEXT Enable smartd on all devices

https://search.nixos.org/options?channel=25.11&query=smartd
...
*** NEXT Move packages to self/pkgs where feasable

I have a bunch of scripts scattered over this repository.
It would make more sense to collect them all together in the pkgs directory.
...
* Restic server
** NEXT Sync data after pruning the backups :bug:

After I locally pruned and checked the data I should sync it automatically the offsite location.

** NEXT Enable notifications for sync script :enhancement:
** NEXT Check offsite data after syncing :enhancement:

After I synced the data to the offsite location I should check with restic that everything is still intact.

** NEXT Enable notifications for check script :enhancement:

** NEXT read data while doing ~restic check~ :enhancement:

It is important to test backups for restore as well.
Restic can do this automatically with the ~check~ command and the flag ~--read-data~[fn:4].
However reading all the data would be a bit overkill for my setup.
Restic has the flag ~--read-data-subset~ which can be used as follows to read a rando amount of data ~--read-data-subset=10G~.
However on a Raspberry with an SD card it is still a problem to write 10 GB of data every week.
Therefore I could create an additional btrfs volume on the SSD and put the temporary files[fn:5] there, reducing the wear on the SD card.
...
* Raspberry Pi 4
** NEXT Build a custom Raspi UEFI :enhancement:
:LOGBOOK:
...
[fn:5] https://restic.readthedocs.io/en/stable/manual_rest.html#temporary-files
[fn:4] https://restic.readthedocs.io/en/stable/045_working_with_repos.html#checking-integrity-and-consistency
