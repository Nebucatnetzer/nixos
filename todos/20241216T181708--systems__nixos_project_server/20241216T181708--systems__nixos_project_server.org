...
*** NEXT Nextcloud issues fixen

- [ ] Server has no maintenance window start time configured. This means resource
  intensive daily background jobs will also be executed during your main usage
  time. We recommend to set it to a time of low usage, so users are less
  impacted by the load caused from these heavy tasks. For more details see the
  documentation. Some headers are not set correctly on your instance

- [ ] The ~X-Robots-Tag~ HTTP header is not set to ~noindex,nofollow~. This is a
  potential security or privacy risk, as it is recommended to adjust this
  setting accordingly. For more details see the documentation.

*** NEXT Add a push mirror to each of my repos

The main repo should always live on my servers but it would be nice to have a backup on GitHub.
It is imperative that they are on GitHub because the AI fuckers are destroying my servers.
...
*** NEXT [#A] Firefly-iii Data Importer, a data importer for Firefly-III.

Available as services.firefly-iii-data-importer.
I need to use the service because the container gets updated faster than my NixOS installtion of firefly and then there is a missmatch between the importer and the actual service.

*** DONE SwissBackup läuft voll
CLOSED: [2025-06-16 Mon 21:18] SCHEDULED: <2025-05-26 Mon>
...
*** NEXT [#B] Set up a better binary cache
...
*** TODO Check if I use self somewhere
...
*** NEXT Test Firefly alternative
SCHEDULED: <2025-11-03 Mon>

Firefly sort of works but it is a bit clunkier than YNAB and I often have a feeling that I'm not fully in control of my finances.
With YNAB I definitly had less of a problem with this.
They author of Firefly says that YNAB encourages living paycheck to paycheck.
I don't think this is true as I had at least two months worth of money saved up.

https://github.com/AshishKapoor/fintrack

**** [[https://actualbudget.org/][Actual | Actual Budget Documentation]]
[2025-06-14 Sat 17:12]

Ich muss dass vielleicht nochmals testen.
Firefly fühlt sich etwas nach zu viel Aufwand an und ich habe nicht das Gefühl, dass ich damit mein Geld super unter Kontrolle habe.
Eher im Gegenteil, ich habe viel mehr Geld ausgegeben als vorher mit YNAB.
...
** DONE Actualbudget für Eleonore aufsetzen
CLOSED: [2025-09-16 Tue 21:17] SCHEDULED: <2025-09-15 Mon>
- State "DONE"       from "NEXT"       [2025-09-16 Tue 21:17]
:LOGBOOK:
CLOCK: [2025-09-14 Sun 13:48]--[2025-09-14 Sun 13:48] =>  0:00
:END:
