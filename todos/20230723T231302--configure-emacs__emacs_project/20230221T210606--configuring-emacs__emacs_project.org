...
*** NEXT autocompletion doesn't work when updating org mode linkes

When I have an org mode link to a file and I would like to point it to a different target I don't get any autocompletion when I press {C-c C-l}. It just displays the string and you have to type everything, hoping that you got the filename right.
When you edit the link directly it works but then sort of breaks the link.
...
*** NEXT Switch to perspective with shortcut (notes, email)

I have a few keybindings like F4 and F5 which open a certain directory or mu4e.
However they always behave the same even when the perspective is already setup.
It might be useful if those commands would check if the perspective is already open and then just switch to it.

#+begin_example
if not perspective("notes"):
  create_perspective("notes")
  open_directory("notes")

switch_to_perspective("notes")
#+end_example
...
*** NEXT Migrate from config to custom
...
*** NEXT [[https://magit.vc/manual/forge.html][Forge User and Developer Manual]]
[2025-09-24 Wed 10:03]

Add Forge support so that I can manage issues and MRs directly from within Emacs.
